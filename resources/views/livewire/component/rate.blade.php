<div class="text-start">
    ...
    <x-jet-dialog-modal wire:model="openRateModal">
        <x-slot name="title">
            <h5 class="text-red-500 text-3x1">Review</h5>
        </x-slot>

        <x-slot name="content">
            <div class="my-4">
                <div x-data="{ temp: 0, orig: null }"
                    @click="orig = temp"
                    class="flex cursor-pointer text-4xl">
                    <template x-if="orig != null">
                        <span class="text-gray-400 hover:text-gray-700"
                            @mouseenter="temp = null"
                    ...
                    <template x-for="item in [1, 2, 3, 4, 5]">
                        <button type="submit"
                            ...
                            @click="$wire.setRating(item)"
                            :class="{ 'text-yellow-300': (temp >= item) }">★</button>
                    </template>
                </div>
                ...
                <p class="text-sm text-gray-500 uppercase">U can report only one time! make sure u report is great</p>
